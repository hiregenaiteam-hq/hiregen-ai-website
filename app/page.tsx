        ...
        <div className="max-w-4xl mx-auto px-4 sm:px-6">
          ...
          <div className="space-y-3 sm:space-y-4 mb-12 sm:mb-16">
            {faqs.map((faq, index) => (
              <motion.div
                key={index}
                className={`bg-white rounded-lg sm:rounded-xl border border-gray-200 overflow-hidden relative ${
                  openFaq === index ? 'border-l-4 border-l-umukozi-teal' : ''
                }`}
                initial={{ opacity: 0, y: 20 }}
                whileInView={{ opacity: 1, y: 0 }}
                transition={{ duration: 0.5, delay: index * 0.1 }}
              ...
              >
                ...
                {openFaq === index && (
                  <div className="absolute left-0 top-0 bottom-0 w-1 bg-umukozi-teal" />
                )}
                ...
                >
                  ...
                  {openFaq === index ? (
                    ...
                    <Plus className="w-4 h-4 sm:w-5 sm:h-5 text-umukozi-orange group-hover:text-umukozi-orange transition-colors duration-300 flex-shrink-0 mt-0.5 sm:mt-0" />
                  )}
                </button>
